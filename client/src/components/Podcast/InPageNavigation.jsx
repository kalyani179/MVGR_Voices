...
const InPageNavigation = ({ routes, defaultHidden = [], defaultActiveTab = 0, children }) => {
    ...
    }, [width, activeTab, defaultActiveTab, isResizeEventAdded]);
            ...
            <div className="relative mb-8 border-b border-grey flex flex-nowrap overflow-x-auto">
                {routes.map((route, index) => (
                    <button
                        ...
                        className={`p-4 px-5 capitalize text-xl tracking-wide ${activeTab === index ? "text-primary font-medium border-b-2 border-primary" : "text-dark-grey"} ${defaultHidden.includes(route) ? "md:hidden" : ""}`}
                    >
                        {route}
                    </button>
                ))}
